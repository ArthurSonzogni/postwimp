...
#ifndef MyFilePager__H
#define MyFilePager__H
...
namespace PolyVox
{
	...
	class MyFilePager : public PagedVolume<VoxelType>::Pager
	{
	public:
		/// Constructor
		MyFilePager(const std::string& strFolderName = ".")
		...
		virtual ~MyFilePager()
		{
			...
			//{
				//POLYVOX_LOG_WARNING_IF(std::remove(iter->c_str()) != 0, "Failed to delete '" << *iter << "' when destroying MyFilePager");
			//}
		...
		virtual void pageIn(const Region& region, typename PagedVolume<VoxelType>::Chunk* pChunk)
		{
			...
			ssFilename << m_strFolderName << "/dat" << "-"
